    ...
    def copy_from_store(cls, src_store, store=None, keys=None, 
        ...
        if store is None:
            ...
            for key in keys:
                ...
                if key == 'img':
                    # arr_i = np.copy(arr)
                    # result = np.zeros([arr_i.shape[0],3,224,224])
                    # for ii in range(len(arr_i)):
                    #     result[ii] = np.array(I_trans(Image.fromarray(arr_i[ii])),dtype=np.uint8).transpose((2,0,1))
                    # arr = np.copy(result)
                    arr = arr.transpose((0,3,1,2))
                if key == 'depth':
                    arr = arr.reshape(arr.shape[0],-1,arr.shape[1],arr.shape[2])
                data[key] = arr
            root = {
                'meta': meta,
                'data': data
