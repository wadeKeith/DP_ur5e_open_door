    ...
    def get_data(self,data_dir_path_abs):
        ...
        for dir in os.listdir(data_dir_path_abs):
            ...
            for i in range(len(obs)):
                ...
                top_rgbd_tmp = np.concatenate([obs[i]['top_img'], obs[i]['top_depth'].reshape(obs[i]['top_depth'].shape[0], obs[i]['top_depth'].shape[1],1)], axis=-1).transpose([2, 0, 1])
                top_ls.append(top_rgbd_tmp)
                right_rgbd_tmp = np.concatenate([obs[i]['right_img'], obs[i]['right_depth'].reshape(obs[i]['right_depth'].shape[0], obs[i]['right_depth'].shape[1],1)], axis=-1).transpose([2, 0, 1])
                right_ls.append(right_rgbd_tmp)
        ...
        top_rgbd = np.array(top_ls)
        right_rgbd = np.array(right_ls)
        state = np.array(state_ls)
        action = np.array(action_ls)
        data['state'] = state
        data['top_rgbd'] = top_rgbd
        # data['top_depth'] = top_depth
        data['right_rgbd'] = right_rgbd
